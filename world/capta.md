...
## Preliminary
There are three things you need to create a choropleth. One is obviously some **data** you want to visualize over a map. For our example we'll be re-using the data in [Our World in Data](https://ourworldindata.org/grapher/annual-healthcare-expenditure-per-capita). The original dataset includes healthcare expenditure data for various years. We filtered it down to only 2014 statistics and we kept the original column names: `Entity`, `Code`, `Year`, ` (constant 2011 international $)`. Here's a look at our CSV file:

```{csv file=data/annual-healthcare-expenditure-per-capita-2014.csv}
```

After providing your **data**, you'll need a **map** as the basis of your choropleth and some **code** to tie your data and the map together. While there's an entire profession of cartographers dedicated to making maps, we believe a small number of pre-made maps can cover a substantial portion of choropleths that you will likely need. For our current example we have [a ready-to-use map of the world showing country boundaries](topojson/ne_110m_admin_0_countries.json). We have a catalog of other pre-made maps to show smaller regions or different boundaries. And if you don't see one that fits your needs, you can create an issue on Github to ask the community for help. Chances are some other people will have the same needs and will be happy to collaborate.
...
Now that we have a data file (in CSV format) and the appropriate map file (in TopoJSON format), the rest of this article will cover how to write a vega-lite file to tie those two together to create a choropleth. [Vega-Lite](https://vega.github.io/vega-lite/) files are written in the JSON format and it reads as a set of specifications instead of programming code. The impatient can jump to [Github](https://github.com/capta-journal/map/blob/master/world/annual-healthcare-expenditure-per-capita.vl.json) and start copying-and-pasting the final code. Or follow the four steps below to see how we iteratively add to our vega-lite specification to get to the final product.
...
```json
{
  "$schema": "https://vega.github.io/schema/vega-lite/v3.json",
  "data": {
    "url": "https://www.capta.studio/journal/map/world/topojson/ne_110m_admin_0_countries.json",
    "format": {
      "type": "topojson",
      "feature": "countries"
    }
...
Vega-Lite specification is a JSON object that identifies itself by its own `$schema` property. The spec above uses the `data` property to point to our ready-to-use map file. Vega-Lite needs to know that this file is in TopoJSON `format.type`. TopoJSON files can encode multiple "features" of a map that show different types of boundaries. For example, a map of the US may show states, counties, zip codes, or other features. Our pre-made world map has only one `countries` feature.
...
So that's it. To create your own world choropleth, all you have to do now is switch in your own data source. Need some inspiration?? [The World Factbook](https://www.cia.gov/library/publications/resources/the-world-factbook/docs/rankorderguide.html) has lots of data about the world, and it's all free!